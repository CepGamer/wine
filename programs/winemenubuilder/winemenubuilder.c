...
static BOOL cleanup_associations(void)
{
    ...
    {
        if(FAILED(winemime_enumerate_registry_extensions(assocKey, &cleanup_associations_handler, &ext_list)))
    ...
    LIST_FOR_EACH_ENTRY_SAFE(ext, next_ext, &ext_list, struct extension_list_entry, entry)
    {
        ...
        if(ext->extension)
            hasChanged |= winemime_remove_mime_association(ext->extension);
...
static HRESULT generate_association_handler(const WCHAR *extensionW, void *user_data)
{
    static const WCHAR openW[] = {'o','p','e','n',0};
    char *mimeTypeA = NULL;
    WCHAR *commandW = NULL;
    ...
    struct generate_associations_data *data = user_data;
    HRESULT ret = S_FALSE;

    if(!winemime_mime_type_for_extension(data->native_mime_types, extensionW, data->packages_dir, &mimeTypeA, &data->has_changed))
        goto end;
    ...
    if (progIdW)
    {
        ...
        goto end_ok; /* no progID => not a file type association */
    ...
    if (mimeProgId)
    {
        ...
        if (wine_rb_get(data->mime_progid_tree, mimeProgId))
        {
        ...
        if (wine_rb_put(data->mime_progid_tree, mimeProgId, &entry->entry))
        {
    ...
    winemime_add_mime_association(extensionW, mimeTypeA, progIdW, data->applications_dir, &data->has_changed);
...
static BOOL generate_associations(const char *xdg_data_home, const char *packages_dir, const char *applications_dir)
{
    ...
    struct generate_associations_data user_data;
    ...
    if (!winemime_build_native_mime_types(xdg_data_home, &nativeMimeTypes))
    {
        WINE_ERR("could not build native MIME types\n");
        return FALSE;
    }

    user_data.applications_dir = applications_dir;
    user_data.packages_dir = packages_dir;
    user_data.native_mime_types = &nativeMimeTypes;
    user_data.mime_progid_tree = &mimeProgidTree;
    user_data.has_changed = FALSE;

    if(FAILED(winemime_enumerate_registry_extensions(HKEY_CLASSES_ROOT, &generate_association_handler, &user_data)))
        WINE_ERR("error enumerating registry extensions\n");
